...
def _generate_camera_positions(means_np: np.ndarray, n_positions: int,
    ...
    def sample_round(alpha, min_sep, min_surface_dist, eps=1e-9):
        ...
        # local median; in a densely cluttered scene with little genuinely
        # open space, that can still land arbitrarily close to one specific
        # surface. This is a separate, absolute safety net.
        nn_dist, _ = tree.query(cands, k=1)
        d_ref = float(np.median(dens)) if dens.size else 0.0
        order = rng.permutation(M)
        for ci in order:
            if len(accepted) >= n_positions:
                break
            if nn_dist[ci] < min_surface_dist:
                continue
            d = dens[ci]
    ...
    accepted = sample_round(cfg.bandpass_alpha, cfg.min_sep_frac,
                            cfg.min_surface_dist_frac * diag)
    ...
    while len(accepted) < n_positions and relax < 3:
        ...
        more = sample_round(cfg.bandpass_alpha * (1.0 + relax),
                            ...
                            (cfg.min_surface_dist_frac / (2 ** relax)) * diag)
...
def _build_poses(positions: list, n_azimuth: int, n_elevation: int,
                 el_min: float = -55.0, el_max: float = 75.0):
    """
