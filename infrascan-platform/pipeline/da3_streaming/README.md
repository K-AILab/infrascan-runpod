...
The `weights/` directory is **git-ignored** — the checkpoints exceed
GitHub's 100 MB per-file limit (`model.safetensors` alone is ~6.3 GB).
Fetch them locally before running the pipeline:

| File | Size |
|---|---:|
| `weights/model.safetensors` | ~6.3 GB (Depth Anything v3) |
| `weights/dino_salad.ckpt` | ~336 MB (DINO-SALAD loop closure) |
| `weights/config.json` | ~3 KB (DA3 config) |

Source: persisted on the team's DGX boxes — ask chan for the current
canonical path (typically under `/home/kail/…` or `/home/chan/…`).
The pipeline expects them at exactly `weights/{model.safetensors,
dino_salad.ckpt,config.json}` relative to this file.
