...
_PZ000_PAT = re.compile(r"(?P<frame>\d{6})_pz000_(?P<yaw>y\d{3})_normal\.(jpg|png)$", re.IGNORECASE)
_MULTI_PAT = re.compile(r"(?P<frame>\d{6})_pz(?P<pitch>\d{3})_(?P<yaw>y\d{3})_normal\.(jpg|png)$", re.IGNORECASE)
@staticmethod
def serpentine_pz000(paths):
        """Serpentine ordering for pz000-only (original) or multi-pitch views.

        For multi-pitch, groups by frame and orders: all pitches for each yaw,
        then serpentine across yaws per frame. This ensures adjacent views in
        the list have maximum spatial overlap (same position, nearby angles).
        """
        # Try multi-pitch first
        g_multi = defaultdict(list)
        g_pz000 = defaultdict(list)
        for p in paths:
            name = os.path.basename(p)
            m_multi = _MULTI_PAT.match(name)
            if m_multi:
                frame = int(m_multi.group("frame"))
                pitch = int(m_multi.group("pitch"))
                yaw = int(m_multi.group("yaw")[1:])
                g_multi[frame].append((pitch, yaw, p))
            else:
                m = _PZ000_PAT.match(name)
                if m:
                    frame = int(m.group("frame"))
                    yaw = int(m.group("yaw")[1:])
                    g_pz000[frame].append((yaw, p))

        # If we have multi-pitch views, use multi-pitch ordering
        # Strategy: sweep all yaws at each pitch level, then move to next pitch.
        # Within each frame: p330(y000..y330), p000(y000..y330), p030(y000..y330)
        # This ensures consecutive views have ~60° horizontal overlap (same as single-pitch).
        # Pitch transitions (p330→p000→p030) have ~60° vertical overlap (90° FOV - 30° pitch step).
        if g_multi:
            out = []
            for frame in sorted(g_multi.keys()):
                # Group by pitch, sweep yaws within each pitch
                by_pitch = defaultdict(list)
                for pitch, yaw, p in g_multi[frame]:
                    by_pitch[pitch].append((yaw, p))

                frame_views = []
                for pitch in sorted(by_pitch.keys()):
                    yaws = sorted(by_pitch[pitch], key=lambda t: t[0])
                    frame_views.extend([p for _, p in yaws])

                if frame % 2 == 1:
                    frame_views.reverse()
                out.extend(frame_views)
            return out

        # Fallback: original pz000-only ordering
        out = []
        for frame in sorted(g_pz000.keys()):
            yaws = sorted(g_pz000[frame], key=lambda t: t[0])
            if frame % 2 == 1:
                yaws.reverse()
            out.extend([p for _, p in yaws])
        return out
...
class DA3_Streaming:
    def __init__(self, image_dir, save_dir, config):
        ...
        self.process_res_method = self.config["Model"].get("process_res_method", "upper_bound_resize")
        self.seed = 42
    ...
    def run(self):
        ...
        paths = glob.glob(os.path.join(self.img_dir, "*.jpg")) + glob.glob(os.path.join(self.img_dir, "*.png"))

        # (A) If you generated pitch=0-only dataset, this is enough:
        self.img_list = serpentine_pz000(paths)

        if len(self.img_list) == 0:
            raise ValueError(f"[DIR EMPTY] No valid images found in {self.img_dir}!")
        print(f"Found {len(self.img_list)} images (serpentine ordered)")

        # Record the exact processing order. camera_poses.txt and frame_<i>.npz
        # are indexed by position in self.img_list (serpentine), which is NOT the
        # alphabetical sort of the views dir — downstream scripts must read this
        # to pair poses/depth with the correct image.
        order_path = os.path.join(self.output_dir, "img_list.txt")
        with open(order_path, "w") as f:
            f.write("\n".join(os.path.basename(p) for p in self.img_list))
        print(f"Wrote processing order to {order_path}")
